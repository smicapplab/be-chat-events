...
const OcrResponseSchema = z.object({
  summary: z
    .string()
    .describe('Summarize the content of the page in 2–3 sentences.'),
  qa: z
    .array(
      z.object({
    ...
    .describe(
      'Generate up to 20 relevant customer-style question-and-answer pairs.',
    ),
});
...
export class OcrService {
  ...
  async getDocumentAnalysis(props: OcrAnalysisDto) {
    ...
    try {
      ...
      if (!docExtract) {
        throw new Error(`DocExtract not found for jobId: ${jobId}`);
      }
      const { data, error } =
        await this.textractService.getDocumentBlocks(jobId);
  ...
  async generateContent(props: GenerateContentDto) {
    ...
    try {
      ...
      const blocks = await this.s3Service.readJsonFileFromS3(
        this.bucket,
        docExtract.blocks,
      );
  ...
  private async safeStartTextExtractAsync(
    params: { fileName: string; bucket: string },
    retries = 5,
  ): Promise<any> {
    try {
  ...
  async recoverErrors(props: RecoverErrorsDto): Promise<void> {
    ...
    if (docTraining) {
      ...
      this.logger.log(
        `Found ${docExtracts.length} unfinished records for docTrainingId ${props.docTrainingId}`,
      );
      ...
      for (const docExtract of docExtracts) {
        ...
        await this.sqsService.sendSQSMessage(
          {
            jobId: docExtract.job_id,
            docTrainingId: props.docTrainingId,
            description: docTraining.description,
          },
          'process-pdf',
          this.chatQueue,
          60 + delayPerRec,
