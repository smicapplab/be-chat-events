...
export class S3Service {
    ...
    async uploadDocument(bucket: string, fileName: string, buffer: Buffer, contentType = "application/pdf"): Promise<void> {
        try {
    ...
    async readJsonFileFromS3(bucket: string, key: string): Promise<any> {
        try {
            ...
            this.logger.error(`Error reading JSON from S3 (${bucket}/${key}):`, error);
            throw error;
        }
    }
    ...
    async getFileBufferFromS3(bucket: string, key: string): Promise<Buffer> {
        try {
            ...
            this.logger.error(`Error reading buffer from S3 (${bucket}/${key}):`, error);
            throw error;
        }
    }
